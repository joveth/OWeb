(function(c, b, e) {
	...
	c.CBPFWSlider.prototype = {
		...
		_config : function() {
			...
			if (this.support) {
				this.transEndEventName = h[d.prefixed("transition")]
						+ ".cbpFWSlider";
			...
			if (this.itemsCount > 1) {
				this.$navPrev = c('<span class="cbp-fwprev">&lt;</span>')
						.hide();
				this.$navNext = c('<span class="cbp-fwnext">&gt;</span>');
				/*c("<nav/>").append(this.$navPrev, this.$navNext).appendTo(
						this.$el);*/
				var l = "";
				for (var g = 0; g < this.itemsCount; ++g) {
					var f = g === this.current ? '<span class="cbp-fwcurrent"></span>'
							: "<span></span>";
					l += f
				}
				var j = c('<div class="cbp-fwdots"/>').append(l).appendTo(
						this.$el);
		...
		_initEvents : function() {
			var f = this;
			if (this.itemsCount > 1) {
				this.$navPrev.on("click.cbpFWSlider", c.proxy(this._navigate,
						this, "previous"));
				this.$navNext.on("click.cbpFWSlider", c.proxy(this._navigate,
						this, "next"));
		...
		_slide : function() {
			...
			if (this.support) {
				this.$list.css("transform", this.support3d ? "translate3d(" + g
						+ "%,0,0)" : "translate(" + g + "%)")
			} else {
		...
		if (typeof g === "string") {
			...
			this
					.each(function() {
						...
						if (!h) {
							a("cannot call methods on cbpFWSlider prior to initialization; attempted to call method '"
									+ g + "'");
							return
						}
